...
openssl genrsa -out certs/ac-key.pem 2048

# CREATE A CSR FROM THE CONFIGURATION FILE AND OUR PRIVATE KEY
openssl req -new -key certs/ac-key.pem -subj "/CN=ac.default.svc" -out ac.csr -config certs/ac_config.txt

# CREATE THE CERT SIGNING THE CSR WITH THE CA CREATED BEFORE
openssl x509 -req -in ac.csr -CA certs/ca.crt -CAkey certs/ca.key -CAcreateserial -out certs/ac-crt.pem

# INJECT CA IN THE WEBHOOK CONFIGURATION
export CA_BUNDLE=$(cat certs/ca.crt | base64 | tr -d '\n')
cat _manifest_.yaml | envsubst > acmanifest.yaml
